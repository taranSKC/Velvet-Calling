 ...
 * 
 * Command to deploy: npx wrangler deploy utils/voice-proxy-worker.ts --name velvet-voice-proxy
 */

export interface Env {
  // Add env variables in your Cloudflare Dashboard:
  // - NEXT_PUBLIC_PAYLOAD_ENCRYPTION = "true"
...
async function getKey(password: string): Promise<CryptoKey> {
  ...
  const rawKey = enc.encode(password.padEnd(32, " ").slice(0, 32));
    ...
    serverWs.addEventListener("message", async (event) => {
      ...
      if (!isInitialized) {
        try {
          ...
          isInitialized = true;
          console.log("Edge Handshake successful. Connecting to xAI...");
          ...
          xaiWs.addEventListener("open", () => {
            // Send the session instructions from Cloudflare to xAI
            xaiWs!.send(
              JSON.stringify({
                type: "session.update",
                session: {
          ...
          serverWs.send(JSON.stringify({ type: "error", error: { message: "Session initialization error" } }));
